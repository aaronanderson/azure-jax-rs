...
public class SearchTest {
	...
	public void testSearch() {
		...
		try {
			...
			JsonObject index = Json
					...
					.add("fields",
							Json.createArrayBuilder().add(Json.createObjectBuilder().add("name", "hotelId").add("type", "Edm.String").add("key", true))
									.add(Json.createObjectBuilder().add("name", "description").add("type", "Edm.String").add("suggestions", true))).build();
			...
			assertNotNull(result);
			result = search.suggestDocuments("hotels", new SuggestQuery().setSearch("mar"));
